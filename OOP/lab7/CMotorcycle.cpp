...
void CMotorcycle::PrintName(ostream& os) const{
...
void CMotorcycle::VehicleDataExport(string FILENAME) const
{
	...
	if (file.is_open()) {
		file << "Motorcycle characteristics:\t" << endl;
		...
		cout << "Motorcycle data exported to " << FILENAME << endl;
	}
	else {
...
VehicleCondition CMotorcycle::VehicleCheck() const
{
	...
	if (ConditionScore <= 0.5) {
		cout << "Your motorcycle is new!" << endl;
		return VehicleCondition::New;
	}
	else if (ConditionScore <= 1.5) {
		cout << "Your motorcycle is in good condition!" << endl;
		return VehicleCondition::Good;
	}
	else if (ConditionScore <= 2.5) {
		cout << "Your motorcycle is in moderate condition." << endl;
		return VehicleCondition::Moderate;
	}
	else {
		cout << "Your motorcycle is broken..." << endl;
