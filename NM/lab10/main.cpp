...
double Func2P(double x) {
	return -((pow(M_PI, 2) * pow(x, 2) - 3) * sin((M_PI * x) / 2) - 6 * M_PI * x * cos((M_PI * x) / 2)) / (4 * sqrt(x));
}

double Func4P(double x) {
	return ((pow(M_PI, 4) * pow(x, 4) - 18 * pow(M_PI, 2) * pow(x, 2) + 9) * sin((M_PI * x) / 2) + (-12 * pow(M_PI, 3) * pow(x, 3) - 12 * M_PI * x) * cos((M_PI * x) / 2)) / (16 * pow(x, (5 / 2)));
}
